...
v0.3.3では、端末の現在地・現在時刻・方位を月の位置へ連動しました。星空画面に月相、明るさ、方位、高度、月の出・月の入りを表示し、OpenClosのDiscord回答では端末座標を送らず、島代表位置による同じ月計算を使います。

全画面地図の操作列には「星座」の入口があり、通常のデスクトップ・タブレット幅でも月と星の画面へ移れます。月選択後の星検索・検索クリア・空白・英語の月検索は、[ブラウザ受入helper](tests/browser-mode-acceptance.mjs)で実際の表示と入力イベントを検証できます。このhelperはNode単体のHTMLテストとは別に、実ブラウザのタブを渡して実行します。

v0.3.2では、v0.3.1の実画面監査で見つかったMapLibre著作権アイコンの繰り返し表示を修正しました。44pxのタッチ領域を保ちつつ、24pxのアイコンを中央に1つだけ表示し、開いたときも出典テキストと重ならないようにしています。

v0.3.1では、公開PWAをネイティブアプリと誤認してBluetooth操作を有効化する判定を修正し、Web版では利用できない操作を明確に無効化しました。地図の著作権表示ボタンも44pxへ拡大し、スマートフォンで押しやすい位置へ調整しています。

v0.3.0では、地図の番号を押すと対応する任務とカメラ導線を直接開くように修正しました。ネイティブ版ではBluetooth LEで近くの友達と任務数・写真タグを端末間同期でき、星空には北極星・ベガ・デネブ・カペラの即時ガイドを追加しています。位置情報、APIキー、写真本体はBluetoothで送信しません。

v0.2.5では、pnpm 11.23以降で旧設定を置き換えた `allowBuilds` を採用し、GitHubのクリーン環境で `esbuild`・`sharp`・`workerd`だけにセットアップ処理を許可します。

v0.2.4では、pnpm 11.24の供給網保護で要求されるビルド依存関係を明示し、GitHubのクリーン環境でも許可したネイティブ依存だけがセットアップ処理を実行できるようにしました。

v0.2.3では、地図下部の通信記録を本当に開閉できるパネルへ変更し、スマートフォン上の重複進捗を整理しました。任務は最初の1件だけ詳細を開き、3地点すべてを一画面で見渡して必要な任務だけ展開できます。GitHub ActionsもNode 24対応の現行メジャーへ更新しています。

v0.2.2では、スマートフォン上の情報密度と操作導線を再設計しました。地図は任務だけを初期表示し、島ガイドの深層情報へ地図内から直接入れます。星空画面は一画面内で空・検索・対象を確認でき、横向きでも地図と星空を切り替えられます。
