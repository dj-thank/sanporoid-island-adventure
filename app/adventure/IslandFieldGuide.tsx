...
export default function IslandFieldGuide({ island, islandName, currentPosition, selectedPoint, open, onOpenChange, embedded = false }: Props) {
  ...
  const availableViews: GuideView[] = anchors.length > 0 ? guideViews : ["current", "deep", "research"];
  ...
  function moveTab(event: KeyboardEvent<HTMLButtonElement>, view: GuideView) {
    ...
    const nextIndex = event.key === "Home" ? 0 : event.key === "End" ? availableViews.length - 1 : (currentIndex + (event.key === "ArrowRight" ? 1 : -1) + availableViews.length) % availableViews.length;
    const nextView = availableViews[nextIndex] ?? "current";
    setActiveView(nextView);
    window.requestAnimationFrame(() => document.getElementById(`field-guide-${nextView}-tab-${island}`)?.focus());
  }

  return (
    <section className={`${styles.fieldGuide} ${embedded ? styles.embedded : ""}`} aria-labelledby="field-guide-title">
      {embedded ? <h2 id="field-guide-title" className={styles.srOnly}>{islandName} 開拓フィールドノート</h2> : <header>
        <div>
        ...
        <div className={styles.guideHeaderActions}>
          ...
          {!embedded && <button type="button" aria-expanded={open} aria-controls={panelId} onClick={() => onOpenChange(!open)}>{open ? "調査レイヤーを閉じる" : "島の調査レイヤーを開く"}</button>}
        </div>
      </header>}
      ...
      <div id={panelId} className={styles.guideBody} hidden={!open && !embedded}>
        ...
        <div className={`${styles.guideTabs} ${anchors.length === 0 ? styles.guideTabsWithoutAnchors : ""}`} role="tablist" aria-label="調査ノートの種類">
        ...
        {anchors.length > 0 && <section id={`field-guide-anchors-${island}`} className={styles.tabPanel} role="tabpanel" aria-labelledby={`field-guide-anchors-tab-${island}`} hidden={activeView !== "anchors"}>
